"""
plot_city/bev_infraction_snapshot.py

Single-pass BEV comparison at the northern curve in Town01 (episode 3).

The northern curve is in episode 3 (spawn x=396, y=20), steps 25–75 of the
expert trajectory (yaw swings from -90° to -168°).  unet fails there at step
49, no_noise at step 66.  cvt_3ch PASSES the curve and fails later at an
intersection (step 145, x=388 y=-3).  Default capture: step 55 (mid-curve).

The script drives through episodes 0→1→2 (1500 steps each for the expert)
then captures the observation at TARGET_EPISODE_STEP within episode 3.
It then feeds the same obs to every BEV generator for comparison.

Generators compared:
  - CVT 3ch L1
  - UNet
  - CVT 6ch Vanilla
  - CVT 6ch Vanilla (no noise)
  - CVT 6ch KDE
  + Real BEV (GT) as reference

Outputs (plot_city/bev_snapshots/town01_north/):
  - real_bev/bev_real_rgb.png           — GT colorized
  - <gen_name>/bev_generated_rgb.png    — colorized output
  - <gen_name>/bev_generated_ch{0,1,2}.png
  - <gen_name>/iou.json
  - panel_comparison.png
  - capture_location.png / capture_info.json

Usage (from anywhere):
    python plot_city/bev_infraction_snapshot.py
    python plot_city/bev_infraction_snapshot.py --target-episode 3 --episode-step 35
"""
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
OUTPUT_DIR     = _REPO_ROOT / 'plot_city/bev_snapshots/town01_north'
CARLA_CFG_PATH = _REPO_ROOT / 'config/carla_config.json'
DEVICE         = 'cuda'

# Driver: real-BEV expert policy (non-temporal, 3-ch) — never fails at this curve
DRIVER_CKPT    = _REPO_ROOT / 'ckpts/ckpt-real-bev/ckpt_latest.pth'

# Northern T-junction: episode 3 (spawn x=396, y=20).
# The road turns from west to south at x≈388-392, y≈-3 to +2.
# Expert (the driver) navigates it at steps 56-74.
# no_noise fails at step 66 (x=390, y=-3); unet already failed at step 49.
# cvt_3ch also fails at this junction (step 145 in its OWN trajectory,
# but the expert policy arrives here at step ~63).
# Capture step 63: expert at x=390.6, y=-0.3 — right at the junction.
DEFAULT_TARGET_EPISODE = 3
DEFAULT_EPISODE_STEP   = 63
...
# Generators to compare: name → (class, infer_fn_key)
# infer_fn_key selects how to build the input batch (see _build_input)
GENERATORS = {
...
def _build_input(gen_name: str, obs: dict) -> dict:
    """Build the inference input dict for a given generator name."""
    ...
    # cvt_6ch_vanilla, cvt_6ch_vanilla_no_noise, cvt_6ch_kde
    return {
...
def run_generator(gen, gen_name: str, obs: dict) -> np.ndarray:
    """Infer BEV. Returns (1, 3, 192, 192) uint8 numpy array {0, 255}."""
    inp = _build_input(gen_name, obs)
    bev = gen.infer(inp)
    return _to_numpy(bev)  # (1, 3, 192, 192)


# ── Colorization & IoU ────────────────────────────────────────────────────────

def colorize_bev(arr: np.ndarray) -> np.ndarray:
    """Binary BEV (H, W, 3) uint8 → semantic-color (H, W, 3) uint8 RGB.
...
def compute_iou(pred_np: np.ndarray, real_bev_np: np.ndarray) -> dict:
    """Per-channel IoU. Both (1, C, H, W) uint8."""
...
def save_bev(gen_name: str, bev_np: np.ndarray, real_bev_np: np.ndarray,
             out_dir: Path) -> dict:
    """Save colorized BEV + per-channel images + iou.json. Returns IoU dict."""
    d = out_dir / gen_name
    d.mkdir(parents=True, exist_ok=True)

    arr = bev_np[0].transpose(1, 2, 0).astype(np.uint8)    # (192, 192, 3)
...
def save_real_bev(real_bev_np: np.ndarray, out_dir: Path):
    """Save GT BEV (first 3 ch) to real_bev/."""
    d = out_dir / 'real_bev'
    d.mkdir(parents=True, exist_ok=True)
...
def build_panel(ious: dict, out_dir: Path):
    """One column per generator: GT real BEV on top, generated BEV below."""
    SZ  = 192
    BAR = 36
    ...
    real_bgr_gt = cv2.imread(str(out_dir / 'real_bev' / 'bev_real_rgb.png'))
...
def save_position_on_map(pos: dict, out_dir: Path,
                         radius_m: float = 400.0, img_size: int = 1024):
    """
    Reconnect to CARLA, reload Town01 (clean background), spawn a top-down
    camera centred on (pos['x'], pos['y']), take one frame and save with
    a red crosshair marking the exact capture location.

    Output: out_dir/capture_location.png
            out_dir/capture_info.json
    """
    import carla  # local import — not required if CARLA server is down

    x, y, yaw = pos.get('x', 0.0), pos.get('y', 0.0), pos.get('yaw', 0.0)

    with open(out_dir / 'capture_info.json', 'w') as f:
        json.dump(pos, f, indent=2)
    print(f'Capture position  x={x:.2f}  y={y:.2f}  yaw={yaw:.1f}°  step={pos.get("step")}')

    carla_cfg = json.load(open(CARLA_CFG_PATH))
    client = carla.Client(carla_cfg['host'], carla_cfg['port'])
    client.set_timeout(60.0)

    # Reload for a clean background (no spawned actors)
    ...
    # Camera height derived from FOV and desired ground radius
    fov = 60.0
    ...
    arr = arr.reshape((raw.height, raw.width, 4))[:, :, :3].copy()   # BGRA→BGR, contiguous

    cam.stop()
    cam.destroy()

    # Draw red crosshair at the centre (= capture position)
...
# ── Known spawn positions (seed=2021, Town01) from trajectory data ─────────────
# ep3 is the northern curve — most models fail at steps 49–145 there.
_EP_SPAWNS = {
    # (spawn_x, spawn_y, spawn_yaw)  — yaw in CARLA convention (0°=+x=north in Town01)
    0: (104.9,   2.0,   0.0),   # ep0: agents go north, cvt_3ch/cvt_6ch/no_noise fail ~step128
    1: (396.3, 318.4, -90.0),
    2: ( 13.6, 330.5, -90.0),
    3: (396.4,  19.9, -90.0),   # ep3: northern T-junction
}


# ── Driver: single CARLA run ──────────────────────────────────────────────────
...
def drive_to_capture(target_episode: int = DEFAULT_TARGET_EPISODE,
    ...
    """
    Spawn the expert directly at target_episode's known start position, drive
    for episode_step steps, and capture.  No need to run through prior episodes.

    Uses obs_configs='cvt_6ch' (480×224) to maximise input richness for all generators.
    Returns (obs_snapshot, position_info) or None on failure.
    """
    carla_cfg = json.load(open(CARLA_CFG_PATH))
    ...
    def make_env():
        ...
        # Bypass endless random-spawn: fix spawn and disable the endless flag
        # so EgoVehicleHandler.reset() uses our transform instead of random choice.
        ...
        raw_env._shuffle_task = False   # prevent reset() from overwriting _task
...
def main():
    ...
    parser.add_argument('--episode-step', type=int, default=DEFAULT_EPISODE_STEP,
                        help='Step within target episode to capture (default: %(default)s)')
    ...
    for gen_name, GenClass in GENERATORS.items():
        ...
        del gen  # free GPU memory before next model
