...
class RlBirdviewWrapper(gym.Wrapper):
    def __init__(self, env, input_states=[], acc_as_action=False, env_id=0, resume=False):
        ...
        if 'rgb' in input_states:
            ...
            observation_space['rear_rgb'] = rgb_space ##added
    ...
    def render(self, mode='human'):
        ...
        '''
        self._render_dict['action_value'] = getattr(self, 'action_value', 0)
        self._render_dict['action_log_probs'] = getattr(self, 'action_log_probs', 0)
        self._render_dict['action_mu'] = getattr(self, 'action_mu', 0)
        self._render_dict['action_sigma'] = getattr(self, 'action_sigma', 0)
    ...
    def process_obs(obs, input_states, train=True):
        ...
        if 'rgb' in input_states:
            ...
            rear_rgb = obs['rear_rgb']['data'] #added
            rear_rgb = np.transpose(rear_rgb, [2, 0, 1])

            obs_dict.update({
                'central_rgb': central_rgb,
                'left_rgb': left_rgb,
                'right_rgb': right_rgb,
                'rear_rgb': rear_rgb ##added
            })
