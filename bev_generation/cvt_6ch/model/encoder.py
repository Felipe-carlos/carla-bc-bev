...
from .dataset_matrices import get_camera_masks
...
class CrossAttention(nn.Module):
    def __init__(self, dim, heads, dim_head, qkv_bias, masks,norm=nn.LayerNorm):
        super().__init__()
        ...
        if masks:
            self.register_buffer('mask', get_camera_masks(), persistent=False)
        else:
            self.mask = None
    ...
    def forward(self, q, k, v, skip=None):
        ...
        if self.mask is not None:
            if dot.shape[0] != self.mask.shape[0]:
                self.register_buffer('mask', get_camera_masks(batch=dot.shape[0]//self.heads), persistent=False)
            dot = dot + self.mask
        att = dot.softmax(dim=-1)

        self.attention_product = att.detach().cpu()  # Detach to avoid gradients through attention
...
class CrossViewAttention(nn.Module):
    def __init__(
        ...
        self.cross_attend = CrossAttention(dim, heads, dim_head, qkv_bias,masks=masks)
        self.skip = skip
...
class Encoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        #print("antes do backbone",image.shape)
        features = [self.down(y) for y in self.backbone(self.norm(image))]
        #print("depois do backbone",features[0].shape)
 
        


        x = self.bev_embedding.get_prior()              # d H W
        #print("bev_emb", x.shape)
        x = repeat(x, '... -> b ...', b=b)              # b d H W
        #print("bev_emb depois do repeat", x.shape)


        for cross_view, feature, layer in zip(self.cross_views, features, self.layers):
            feature = rearrange(feature, '(b n) ... -> b n ...', b=b, n=n)
            #print("cvt - feature rerenage", feature.shape)

            x = cross_view(x, self.bev_embedding, feature, I_inv, E_inv)
            #print("cross view",x.shape)

            x = layer(x)
            #print("saida layer", x.shape)
