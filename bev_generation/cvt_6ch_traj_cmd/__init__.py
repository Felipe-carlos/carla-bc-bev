...
from .model.model_builder import ModelBuilderTrajCmd
import torch
import os


class CVT_6chTrajCmd(IBEVGenerator):
    def __init__(self, model_path: str = None, device='cuda', use_eval=True):
        self.device = device
        if model_path is None:
            main = os.getcwd()
            model_path = os.path.join(main, 'bev_generation/cvt_6ch_traj_cmd/ckpts/ckpt_49.pth')
